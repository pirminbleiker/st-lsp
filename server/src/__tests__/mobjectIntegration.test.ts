...
function makeExtractedDoc(
  ...
  const doc = TextDocument.create(`file:///fixtures/${filename.replace(/\.[^.]+$/, '.st')}`, 'st', 1, result.source);
  return { doc, result };
}
...
describe('Completion Handler: mobject-core POUs', () => {
  ...
  describe('cross-file completion with extracted mobject-core workspace', () => {
    ...
    it('completion does not crash with mobject-core workspace', () => {
      const mockIndex = { getProjectFiles: () => extractedUris, getLibraryRefs: () => [], getLibrarySymbols: () => [] } as unknown as WorkspaceIndex;
